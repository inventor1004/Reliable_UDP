...
// platform detection
//> determine which platform the user currently use 
#define PLATFORM_WINDOWS  1
#define PLATFORM_MAC      2
#define PLATFORM_UNIX     3

//> _WIN32, __APPLE__ these "Preprocessor Macros" are defined by preprocessor before the compile time
//> So, the compiler can determine which platform the user is currently using by checking a preprocessor mecros
#if defined(_WIN32)
...
//> if the user is running the program under the Mac or Unix the below headers are required
...
/// <summary>
/// net name space includes
///			1. Address class
///			2. Socket class
///			3. Connection class
///				3.1 ReliableConnection sub class
///			4. ReliabilitySystem class
/// </summary>
namespace net
{
...
#include <unistd.h>
	...
	public:
		...
		//> Take for ip address numbers and a port number, and initialize the address and port property 
	...
	public:
		...
		bool Open(unsigned short port)
		{
			...
			// SOCK_DGRAM: Means using Data GRAM type socket (= UDP)
			// IPPROTO_UDP: Select the UDP as a protocol
		...
		//> send the packet to the destinaion address
		bool Send(const Address& destination, const void* data, int size)
		{
			assert(data);			// Check whether data is null or not
			assert(size > 0);		// Check whether the data size is bigger than 0

			// Check whether the socket sets properly or not
			if (socket == 0)
				return false;

			// Check whether the destination IP address and port number are set properly.
			assert(destination.GetAddress() != 0);					
			assert(destination.GetPort() != 0);

			// Save destination oject information
			sockaddr_in address;
			address.sin_family = AF_INET;
			...
			// Check whether the size of the data you want to send matches the size of the data actually sent.
		...
		int Receive(Address& sender, void* data, int size)
		{
			...
			// Check whether the platform is Window or not
#if PLATFORM == PLATFORM_WINDOWS
			typedef int socklen_t;
#endif
			// Preparation for storing information about the address from which the packet was sent
			...
			unsigned int address = ntohl(from.sin_addr.s_addr);  // Store the IP address and port number info
			unsigned short port = ntohs(from.sin_port);

			sender = Address(address, port);					 // pass the information to overloaded constructor

			return received_bytes;								 // return the size of packet
		}
	...
	public:
		...
		//> Accumulate the time and disconnect a connection when it time out
		...
		//> check whether the state is running or not
		//		>> If not, finish the function by 'assert'
		//> check whether the address is reliable or not
		//		>> If not, return false
		//		>> If so, copy the sent data
		virtual bool SendPacket(const unsigned char data[], int size)
		{
			assert(running);
			if (address.GetAddress() == 0)
				return false;
			unsigned char packet[PacketSizeHack + 4];					// Prepare the buffer
																		//> Check protocolID so that the system can realize which protocol a packet uses
																		//	 >> protocolID is composed by 32bits long
																		//	 >> Since only 1 byte can be stored in each char index, the protocol ID is stored in 8 bits at a time
			packet[0] = (unsigned char)(protocolId >> 24);				
			packet[1] = (unsigned char)((protocolId >> 16) & 0xFF);		// The reason for using & 0xFF is to extract only the lower 8 bits from a specific variable.
			packet[2] = (unsigned char)((protocolId >> 8) & 0xFF);
			packet[3] = (unsigned char)((protocolId) & 0xFF);

			std::memcpy(&packet[4], data, size);						// make as an one packet which includes packetID, data, and its size
			return socket.Send(address, packet, size + 4);				// send the packet to the set address (+ 4 = packetID size)
		}
	...
	public:
		 ...
		 * Definition	: sort the packet based on the sequence number of each packet
		 * Parameter	: const PacketData& p		
		 *				  >> Includes packets and sequence number
		...
		void insert_sorted(const PacketData& p, unsigned int max_sequence)
		{
			...
			else
			{
				...
				{
					// Check whether the current packet's squence number is less than the first data in the queue
					// If so, push the packet to the front
					push_front(p);
				}
				else if (sequence_more_recent(p.sequence, back().sequence, max_sequence))
				{
					// Check whether the current packet's squence number is bigger than the last data in the queue
					// If so, push the packet to the end
					push_back(p);
				}
				else
				{
					// Check all packets in the queue and find the place to place a current packet
		...
		// Check whether the packets are placed in a right order based on the sequnce number
	...
	*/
		...
		//  Acknowledgment bits are bit patterns used to track received packets.
		// Each bit indicates whether a packet with a specific sequence number was received
		//  For example, if a packet with a specific sequence number is received, the bit at
		// the location corresponding to that sequence number is set to 1.
		...
		//  Check the sequence number of the received packet, and 
		// if the number is included in ack or ack_bits, 
		// the packet is considered to have been received normally and check
		// and manage whether the packets sent through the network have accurately reached the recipient.
